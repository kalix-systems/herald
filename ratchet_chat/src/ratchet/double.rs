...
pub const MAX_SKIP: Counter = 1000;
...
impl DoubleRatchet {
    ...
    fn try_decrypt<KS: KeyStore>(
        ...
        if self.should_fetch_key(store, h)? {
            ...
            let skip = h.n.wrapping_sub(self.nr);
            if skip <= MAX_SKIP {
                let (ckr, mut mks) = Self::skip_message_keys(
                    self.ckr.as_ref().ok_or(DecryptError::Uninit)?,
                    skip as usize,
                );
            ...
            if skip > MAX_SKIP {
                ...
                let (ckr, mut mks) = Self::skip_message_keys(&ckr, skip as usize);
                let mk = mks.pop().unwrap();
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Ser, De, Getters, Hash)]
pub struct Header {
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Error, Ser, De, Hash)]
