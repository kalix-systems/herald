...
fn name(id: UserId) -> Result<Option<String>, HErr> {
    let db = Database::get()?;
...
pub(crate) fn set_name(id: UserId, name: Option<&str>) -> Result<(), HErr> {
    let db = Database::get()?;
...
fn profile_picture(id: UserId) -> Result<Option<String>, HErr> {
    let db = Database::get()?;
...
pub(crate) fn set_color(id: UserId, color: u32) -> Result<(), HErr> {
    let db = Database::get()?;
    db.execute(
...
fn contact_exists(id: UserId) -> Result<bool, HErr> {
    let db = Database::get()?;
...
/// Gets contact status
fn status(id: UserId) -> Result<ContactStatus, HErr> {
    let db = Database::get()?;
...
pub(crate) fn by_user_id(user_id: UserId) -> Result<Contact, HErr> {
    let db = Database::get()?;
