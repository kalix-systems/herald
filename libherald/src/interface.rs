...
pub struct ConversationBuilderUsersQObject {}

pub struct ConversationBuilderUsersEmitter {
    qobject: Arc<AtomicPtr<ConversationBuilderUsersQObject>>,
    filter_changed: fn(*mut ConversationBuilderUsersQObject),
    new_data_ready: fn(*mut ConversationBuilderUsersQObject),
}

impl ConversationBuilderUsersEmitter {
    /// Clone the emitter
    ///
    /// The emitter can only be cloned when it is mutable. The emitter calls
    /// into C++ code which may call into Rust again. If emmitting is possible
    /// from immutable structures, that might lead to access to a mutable
    /// reference. That is undefined behaviour and forbidden.
    pub fn clone(&mut self) -> ConversationBuilderUsersEmitter {
        ConversationBuilderUsersEmitter {
            qobject: self.qobject.clone(),
    ...
    fn clear(&self) {
        let n: *const ConversationBuilderUsersQObject = null();
        self.qobject
            .store(n as *mut ConversationBuilderUsersQObject, Ordering::SeqCst);
    }
...
#[derive(Clone)]
pub struct ConversationBuilderUsersList {
    qobject: *mut ConversationBuilderUsersQObject,
    layout_about_to_be_changed: fn(*mut ConversationBuilderUsersQObject),
    layout_changed: fn(*mut ConversationBuilderUsersQObject),
    data_changed: fn(*mut ConversationBuilderUsersQObject, usize, usize),
    begin_reset_model: fn(*mut ConversationBuilderUsersQObject),
    end_reset_model: fn(*mut ConversationBuilderUsersQObject),
    begin_insert_rows: fn(*mut ConversationBuilderUsersQObject, usize, usize),
    end_insert_rows: fn(*mut ConversationBuilderUsersQObject),
    begin_move_rows: fn(*mut ConversationBuilderUsersQObject, usize, usize, usize),
    end_move_rows: fn(*mut ConversationBuilderUsersQObject),
    begin_remove_rows: fn(*mut ConversationBuilderUsersQObject, usize, usize),
    end_remove_rows: fn(*mut ConversationBuilderUsersQObject),
}

impl ConversationBuilderUsersList {
    pub fn layout_about_to_be_changed(&mut self) {
        (self.layout_about_to_be_changed)(self.qobject);
    }
    pub fn layout_changed(&mut self) {
...
pub trait ConversationBuilderUsersTrait {
    fn new(emit: ConversationBuilderUsersEmitter, model: ConversationBuilderUsersList) -> Self;
    fn emit(&mut self) -> &mut ConversationBuilderUsersEmitter;
...
#[no_mangle]
pub extern "C" fn conversation_builder_users_new(
    conversation_builder_users: *mut ConversationBuilderUsersQObject,
    conversation_builder_users_filter_changed: fn(*mut ConversationBuilderUsersQObject),
    conversation_builder_users_new_data_ready: fn(*mut ConversationBuilderUsersQObject),
    conversation_builder_users_layout_about_to_be_changed: fn(*mut ConversationBuilderUsersQObject),
    conversation_builder_users_layout_changed: fn(*mut ConversationBuilderUsersQObject),
    conversation_builder_users_data_changed: fn(*mut ConversationBuilderUsersQObject, usize, usize),
    conversation_builder_users_begin_reset_model: fn(*mut ConversationBuilderUsersQObject),
    conversation_builder_users_end_reset_model: fn(*mut ConversationBuilderUsersQObject),
    conversation_builder_users_begin_insert_rows: fn(
        *mut ConversationBuilderUsersQObject,
        usize,
        usize,
    ),
    conversation_builder_users_end_insert_rows: fn(*mut ConversationBuilderUsersQObject),
    conversation_builder_users_begin_move_rows: fn(
        *mut ConversationBuilderUsersQObject,
        usize,
        usize,
        usize,
    ),
    conversation_builder_users_end_move_rows: fn(*mut ConversationBuilderUsersQObject),
    conversation_builder_users_begin_remove_rows: fn(
        *mut ConversationBuilderUsersQObject,
        usize,
        usize,
    ),
    conversation_builder_users_end_remove_rows: fn(*mut ConversationBuilderUsersQObject),
) -> *mut ConversationBuilderUsers {
    let conversation_builder_users_emit = ConversationBuilderUsersEmitter {
        qobject: Arc::new(AtomicPtr::new(conversation_builder_users)),
        filter_changed: conversation_builder_users_filter_changed,
        new_data_ready: conversation_builder_users_new_data_ready,
    };
    let model = ConversationBuilderUsersList {
        qobject: conversation_builder_users,
        layout_about_to_be_changed: conversation_builder_users_layout_about_to_be_changed,
        layout_changed: conversation_builder_users_layout_changed,
        data_changed: conversation_builder_users_data_changed,
        begin_reset_model: conversation_builder_users_begin_reset_model,
        end_reset_model: conversation_builder_users_end_reset_model,
        begin_insert_rows: conversation_builder_users_begin_insert_rows,
        end_insert_rows: conversation_builder_users_end_insert_rows,
        begin_move_rows: conversation_builder_users_begin_move_rows,
        end_move_rows: conversation_builder_users_end_move_rows,
        begin_remove_rows: conversation_builder_users_begin_remove_rows,
        end_remove_rows: conversation_builder_users_end_remove_rows,
    };
    let d_conversation_builder_users =
        ConversationBuilderUsers::new(conversation_builder_users_emit, model);
    Box::into_raw(Box::new(d_conversation_builder_users))
}

#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_free(ptr: *mut ConversationBuilderUsers) {
    Box::from_raw(ptr).emit().clear();
}

#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_filter_get(
    ptr: *const ConversationBuilderUsers,
    p: *mut QString,
    set: fn(*mut QString, *const c_char, c_int),
) {
    let o = &*ptr;
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_filter_set(
    ptr: *mut ConversationBuilderUsers,
    v: *const c_ushort,
    len: c_int,
) {
    let o = &mut *ptr;
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_filter_set_none(
    ptr: *mut ConversationBuilderUsers,
) {
    let o = &mut *ptr;
    o.set_filter(None);
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_clear_filter(
    ptr: *mut ConversationBuilderUsers,
) {
    let o = &mut *ptr;
    o.clear_filter()
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_row_count(
    ptr: *const ConversationBuilderUsers,
) -> c_int {
    to_c_int((&*ptr).row_count())
}
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_insert_rows(
    ptr: *mut ConversationBuilderUsers,
    row: c_int,
    count: c_int,
) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_remove_rows(
    ptr: *mut ConversationBuilderUsers,
    row: c_int,
    count: c_int,
) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_can_fetch_more(
    ptr: *const ConversationBuilderUsers,
) -> bool {
    (&*ptr).can_fetch_more()
}
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_fetch_more(ptr: *mut ConversationBuilderUsers) {
    (&mut *ptr).fetch_more()
}

#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_sort(
    ptr: *mut ConversationBuilderUsers,
    column: u8,
    order: SortOrder,
) {
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_data_color(
    ptr: *const ConversationBuilderUsers,
    row: c_int,
) -> COption<u32> {
    let o = &*ptr;
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_data_matched(
    ptr: *const ConversationBuilderUsers,
    row: c_int,
) -> bool {
    let o = &*ptr;
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_data_name(
    ptr: *const ConversationBuilderUsers,
    row: c_int,
    d: *mut QString,
    set: fn(*mut QString, *const c_char, len: c_int),
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_data_profile_picture(
    ptr: *const ConversationBuilderUsers,
    row: c_int,
    d: *mut QString,
    set: fn(*mut QString, *const c_char, len: c_int),
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_data_selected(
    ptr: *const ConversationBuilderUsers,
    row: c_int,
) -> bool {
    let o = &*ptr;
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_set_data_selected(
    ptr: *mut ConversationBuilderUsers,
    row: c_int,
    v: bool,
) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn conversation_builder_users_data_user_id(
    ptr: *const ConversationBuilderUsers,
    row: c_int,
    d: *mut QString,
    set: fn(*mut QString, *const c_char, len: c_int),
...
pub struct GlobalMessageSearchQObject {}

pub struct GlobalMessageSearchEmitter {
    qobject: Arc<AtomicPtr<GlobalMessageSearchQObject>>,
    regex_search_changed: fn(*mut GlobalMessageSearchQObject),
    search_pattern_changed: fn(*mut GlobalMessageSearchQObject),
    new_data_ready: fn(*mut GlobalMessageSearchQObject),
}

impl GlobalMessageSearchEmitter {
    /// Clone the emitter
    ///
    /// The emitter can only be cloned when it is mutable. The emitter calls
    /// into C++ code which may call into Rust again. If emmitting is possible
    /// from immutable structures, that might lead to access to a mutable
    /// reference. That is undefined behaviour and forbidden.
    pub fn clone(&mut self) -> GlobalMessageSearchEmitter {
        GlobalMessageSearchEmitter {
            qobject: self.qobject.clone(),
    ...
    fn clear(&self) {
        let n: *const GlobalMessageSearchQObject = null();
        self.qobject
            .store(n as *mut GlobalMessageSearchQObject, Ordering::SeqCst);
    }
...
#[derive(Clone)]
pub struct GlobalMessageSearchList {
    qobject: *mut GlobalMessageSearchQObject,
    layout_about_to_be_changed: fn(*mut GlobalMessageSearchQObject),
    layout_changed: fn(*mut GlobalMessageSearchQObject),
    data_changed: fn(*mut GlobalMessageSearchQObject, usize, usize),
    begin_reset_model: fn(*mut GlobalMessageSearchQObject),
    end_reset_model: fn(*mut GlobalMessageSearchQObject),
    begin_insert_rows: fn(*mut GlobalMessageSearchQObject, usize, usize),
    end_insert_rows: fn(*mut GlobalMessageSearchQObject),
    begin_move_rows: fn(*mut GlobalMessageSearchQObject, usize, usize, usize),
    end_move_rows: fn(*mut GlobalMessageSearchQObject),
    begin_remove_rows: fn(*mut GlobalMessageSearchQObject, usize, usize),
    end_remove_rows: fn(*mut GlobalMessageSearchQObject),
}

impl GlobalMessageSearchList {
    pub fn layout_about_to_be_changed(&mut self) {
        (self.layout_about_to_be_changed)(self.qobject);
    }
    pub fn layout_changed(&mut self) {
...
pub trait GlobalMessageSearchTrait {
    fn new(emit: GlobalMessageSearchEmitter, model: GlobalMessageSearchList) -> Self;
    fn emit(&mut self) -> &mut GlobalMessageSearchEmitter;
...
#[no_mangle]
pub extern "C" fn global_message_search_new(
    global_message_search: *mut GlobalMessageSearchQObject,
    global_message_search_regex_search_changed: fn(*mut GlobalMessageSearchQObject),
    global_message_search_search_pattern_changed: fn(*mut GlobalMessageSearchQObject),
    global_message_search_new_data_ready: fn(*mut GlobalMessageSearchQObject),
    global_message_search_layout_about_to_be_changed: fn(*mut GlobalMessageSearchQObject),
    global_message_search_layout_changed: fn(*mut GlobalMessageSearchQObject),
    global_message_search_data_changed: fn(*mut GlobalMessageSearchQObject, usize, usize),
    global_message_search_begin_reset_model: fn(*mut GlobalMessageSearchQObject),
    global_message_search_end_reset_model: fn(*mut GlobalMessageSearchQObject),
    global_message_search_begin_insert_rows: fn(*mut GlobalMessageSearchQObject, usize, usize),
    global_message_search_end_insert_rows: fn(*mut GlobalMessageSearchQObject),
    global_message_search_begin_move_rows: fn(*mut GlobalMessageSearchQObject, usize, usize, usize),
    global_message_search_end_move_rows: fn(*mut GlobalMessageSearchQObject),
    global_message_search_begin_remove_rows: fn(*mut GlobalMessageSearchQObject, usize, usize),
    global_message_search_end_remove_rows: fn(*mut GlobalMessageSearchQObject),
) -> *mut GlobalMessageSearch {
    let global_message_search_emit = GlobalMessageSearchEmitter {
    ...
    let model = GlobalMessageSearchList {
    ...
    let d_global_message_search = GlobalMessageSearch::new(global_message_search_emit, model);
    Box::into_raw(Box::new(d_global_message_search))
}

#[no_mangle]
pub unsafe extern "C" fn global_message_search_free(ptr: *mut GlobalMessageSearch) {
    Box::from_raw(ptr).emit().clear();
}

#[no_mangle]
pub unsafe extern "C" fn global_message_search_regex_search_get(
    ptr: *const GlobalMessageSearch,
) -> COption<bool> {
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_regex_search_set(
    ptr: *mut GlobalMessageSearch,
    v: bool,
) {
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_regex_search_set_none(
    ptr: *mut GlobalMessageSearch,
) {
    let o = &mut *ptr;
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_search_pattern_get(
    ptr: *const GlobalMessageSearch,
    p: *mut QString,
    set: fn(*mut QString, *const c_char, c_int),
) {
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_search_pattern_set(
    ptr: *mut GlobalMessageSearch,
    v: *const c_ushort,
    len: c_int,
) {
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_search_pattern_set_none(
    ptr: *mut GlobalMessageSearch,
) {
    let o = &mut *ptr;
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_clear_search(ptr: *mut GlobalMessageSearch) {
    let o = &mut *ptr;
    o.clear_search()
}

#[no_mangle]
pub unsafe extern "C" fn global_message_search_row_count(ptr: *const GlobalMessageSearch) -> c_int {
    to_c_int((&*ptr).row_count())
}

#[no_mangle]
pub unsafe extern "C" fn global_message_search_insert_rows(
    ptr: *mut GlobalMessageSearch,
    row: c_int,
    count: c_int,
) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_remove_rows(
    ptr: *mut GlobalMessageSearch,
    row: c_int,
    count: c_int,
) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_can_fetch_more(
    ptr: *const GlobalMessageSearch,
) -> bool {
    (&*ptr).can_fetch_more()
}
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_fetch_more(ptr: *mut GlobalMessageSearch) {
    (&mut *ptr).fetch_more()
}

#[no_mangle]
pub unsafe extern "C" fn global_message_search_sort(
    ptr: *mut GlobalMessageSearch,
    column: u8,
    order: SortOrder,
) {
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_data_author(
    ptr: *const GlobalMessageSearch,
    row: c_int,
    d: *mut QString,
    set: fn(*mut QString, *const c_char, len: c_int),
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_data_body(
    ptr: *const GlobalMessageSearch,
    row: c_int,
    d: *mut QString,
    set: fn(*mut QString, *const c_char, len: c_int),
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_data_conversation(
    ptr: *const GlobalMessageSearch,
    row: c_int,
    d: *mut QByteArray,
    set: fn(*mut QByteArray, *const c_char, len: c_int),
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_data_has_attachments(
    ptr: *const GlobalMessageSearch,
    row: c_int,
) -> COption<bool> {
    let o = &*ptr;
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_data_msg_id(
    ptr: *const GlobalMessageSearch,
    row: c_int,
    d: *mut QByteArray,
    set: fn(*mut QByteArray, *const c_char, len: c_int),
...
#[no_mangle]
pub unsafe extern "C" fn global_message_search_data_time(
    ptr: *const GlobalMessageSearch,
    row: c_int,
) -> COption<i64> {
    let o = &*ptr;
