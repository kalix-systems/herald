...
pub trait UtilsTrait {
    ...
    fn image_dimensions(&self, path: String) -> String;

    fn is_valid_rand_id(&self, bs: &[u8]) -> bool;

    fn save_file(&self, fpath: String, target_path: String) -> bool;
}
...
#[no_mangle]
pub unsafe extern "C" fn utils_compare_byte_array(ptr: *const Utils, bs1_str: *const c_char, bs1_len: c_int, bs2_str: *const c_char, bs2_len: c_int) -> bool {
    let obj = &*ptr;
    let bs1 = { qba_slice!(bs1_str, bs1_len) };
    let bs2 = { qba_slice!(bs2_str, bs2_len) };
    obj.compare_byte_array(
    bs1,
    bs2,
    )
}
...
#[no_mangle]
pub unsafe extern "C" fn utils_image_dimensions(ptr: *const Utils, path_str: *const c_ushort, path_len: c_int, data: *mut QString, set: fn(*mut QString, str_: *const c_char, len: c_int)) {
    let obj = &*ptr;
    let mut path = String::new();
    set_string_from_utf16(&mut path, path_str, path_len);
    let ret = obj.image_dimensions(
    path,
    )
    ;
...
#[no_mangle]
pub unsafe extern "C" fn utils_is_valid_rand_id(ptr: *const Utils, bs_str: *const c_char, bs_len: c_int) -> bool {
    let obj = &*ptr;
    let bs = { qba_slice!(bs_str, bs_len) };
    obj.is_valid_rand_id(
    bs,
    )
}

#[no_mangle]
pub unsafe extern "C" fn utils_save_file(ptr: *const Utils, fpath_str: *const c_ushort, fpath_len: c_int, target_path_str: *const c_ushort, target_path_len: c_int) -> bool {
    let obj = &*ptr;
