...
/// Adds configuration.
pub(crate) fn add<D>(mut conn: D, builder: ConfigBuilder) -> Result<Config, HErr>
where
    ...
    D: Deref<Target = Database> + DerefMut,
{
    ...
    let builder = crate::config::ConfigBuilder::new(uid, sig::KeyPair::gen_new());

    add(conn, builder).expect("Failed to create config")
}
