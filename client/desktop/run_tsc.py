...
for file in set(tsfiles):
    file_match = ts_pattern.match(file)
    ...
    src_js_parts = list(file_match.groups())
    src_js_parts[-1] = ".js"
    src_js_file = "".join(src_js_parts)

    dest_js_parts = list(file_match.groups())
    dest_js_parts[-3] = "/js/"
    dest_js_parts[-1] = ".mjs"
...
for src, dest in mjs_files.items():
...
diff = src_to_dest.values().difference(qrc_mjs_files)
